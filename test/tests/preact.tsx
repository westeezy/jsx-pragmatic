    ...
    it('should render a basic element as a Preact element with a tag name, dynamic attribute, and inner text', () => {
        ...
        const Preact = {
            h: (name, props, ...children) => {
                ...
                if (name !== 'button') {
                    throw new Error(
                        `Expected Preact tag name to be button, got ${ name }`
                    );
                }
        ...
        const Preact = {
            h: (name, props, ...children) => {
                ...
                if (name === 'section' && children.length !== 1) {
                    throw new Error(
                        `Expected a single child, got ${ children.length }`
                    );
                }
        ...
        const Preact = {
            h: (el, props, ...children) => {
                if (typeof el === 'function') {
                    ...
                    if (el !== 'button') {
                        throw new Error(
                            `Expected Preact tag name to be button, got ${ el }`
                        );
                    }
            ...
            h: (el, props, ...children) => {
                if (typeof el === 'function') {
                    ...
                    if (result !== renderedElement) {
                        throw new Error(
                            `Expected el function to return children`
                        );
                    }
    ...
    it('should render a basic element as a Preact element with innerHTML', () => {
        ...
        const Preact = {
            h: (name, props) => {
                ...
                if (name !== 'style') {
                    throw new Error(
                        `Expected Preact tag name to be style, got ${ name }`
                    );
                }
